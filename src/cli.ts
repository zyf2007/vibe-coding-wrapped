...
import { detectAgentInputs } from "./agents.js";
...
const usageText = `Usage:
  vibe-wrapped build  (--year YYYY | --month YYYY-MM) [-i PATH]... --out REPORT_DIR
  vibe-wrapped render [--year YYYY | --month YYYY-MM] [-i PATH]... [--clean] [--theme official|compact|PATH] --out REPORT_DIR
  vibe-wrapped serve  [--year YYYY | --month YYYY-MM] [-i PATH]... [--clean] [--theme official|compact|PATH] [--host HOST] [--port PORT] --out REPORT_DIR
...
function validateGenerationOptions(options: Options): void {
  if (Boolean(options.year) === Boolean(options.month)) fail("Generation requires exactly one of --year or --month");
...
async function generate(options: Options, output: string): Promise<void> {
  ...
  const paths = options.inputs.length ? options.inputs : [process.env.CODEX_HOME ? resolve(process.env.CODEX_HOME) : join(homedir(), ".codex")];
  const inputs = await detectAgentInputs(paths, (message) => console.error(message));
  if (!inputs.length) throw new Error("No supported agent data directories were detected");
  const codexRoots = inputs.filter((input) => input.type === "codex").map((input) => input.root);
  const facts = await readCodexFacts(codexRoots, scope, (message) => console.error(message), { bypassCache: options.clean });
