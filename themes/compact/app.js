...
function tokenAmount(input) { const amount = Number(input) || 0; const root = el("div", "token-amount"); append(root, el("strong", "", `${tokenDecimal.format(amount / 1_000_000)}M`), el("small", "", `≈ ${tokenDecimal.format(amount / 100_000_000)} 亿`)); return root; }
...
function stats(items) { const root = el("div", "stats"); items.forEach(([label, result, kind]) => { const item = el("div", "stat"); append(item, kind === "token" ? tokenAmount(result) : el("strong", "", format(result)), el("span", "", label)); root.append(item); }); return root; }
...
function quote(label, record) { if (!record) return null; const root = el("article", "quote"); append(root, el("small", "", label), el("blockquote", "", record.excerpt || `${record.charCount || 0} 字 Prompt`), el("small", "quote-source", `${record.localDateTime || "时间未知"} · ${promptSource(record)}`)); return root; }
...
function interactiveTooltip(target, text) { target.tabIndex = 0; if (!target.hasAttribute("aria-label")) target.setAttribute("aria-label", text); if (["I", "SPAN"].includes(target.tagName)) target.setAttribute("role", "img"); target.addEventListener("pointerenter", () => tooltipFor(target, text)); target.addEventListener("pointerleave", hideTooltip); target.addEventListener("focus", () => tooltipFor(target, text)); target.addEventListener("blur", hideTooltip); }

function renderHeader(bundle) {
  const totals = value(bundle.overview.totals, {}); const root = el("header", "report-head");
  append(root, el("p", "eyebrow", "VIBE CODING WRAPPED · LOCAL AGENTS"), el("h1", "", bundle.manifest.report.period.value), el("p", "lede", "一份适合嵌入博客的紧凑活动记录。所有数字来自本地 Agent 日志，可独立静态托管。"), stats([["提示词", totals.prompts], ["活跃日", totals.activeDays], ["工具调用", totals.toolCalls], ["Token", totals.totalTokens, "token"], ["新增代码行", totals.addedLines]]));
  return root;
}

function renderRhythm(bundle) {
  const root = section("TIME", "时间与节奏"); const days = value(bundle.activity.calendar.days, []); const maxDay = Math.max(1, ...days.map((day) => day.prompts)); const calendar = el("div", "calendar");
  calendar.classList.add(days.length < 100 ? "calendar-short" : "calendar-long");
  const offset = days.length ? new Date(`${days[0].codingDay}T00:00:00Z`).getUTCDay() : 0;
  for (let index = 0; index < offset; index += 1) { const pad = el("i", "calendar-pad"); pad.setAttribute("aria-hidden", "true"); calendar.append(pad); }
  days.forEach((day) => { const cell = el("i"); const label = `${day.codingDay} · ${day.prompts} prompts`; cell.style.setProperty("--level", String(Math.ceil(day.prompts / maxDay * 8))); interactiveTooltip(cell, label); calendar.append(cell); });
...
function renderPrompts(bundle) {
  ...
  const showWord = (term) => { const context = contexts.get(term); detail.replaceChildren(el("strong", "", `${term} · ${context?.count || 0} 次`), el("span", "", context?.representative?.excerpt || "当前隐私模式不包含原文"), el("small", "quote-source", context?.representative ? promptSource(context.representative) : "来源未知")); };
  terms.forEach((item, index) => { const button = el("button", "word", item.term); button.style.setProperty("--weight", String(Math.max(1, Math.ceil(item.count / max * 7)))); interactiveTooltip(button, `${item.term} · ${item.count} 次 · ${item.promptCount} 条 Prompt`); button.addEventListener("click", () => showWord(item.term)); cloud.append(button); if (index === 0) showWord(item.term); });
...
function languageList(items) { const root = el("div", "language-bars"); items.slice(0, 8).forEach((item) => { const row = el("div", "language-row"); const track = el("span", "language-track"); const bar = el("i"); const label = `${item.language} · ${format(item.addedLines)} 行 · ${percent(item.share)}`; bar.style.setProperty("--share", String(item.share)); interactiveTooltip(bar, label); track.append(bar); append(row, el("span", "", item.language), track, el("span", "", format(item.addedLines))); root.append(row); }); return root; }
...
function renderDelivery(bundle) {
  const root = section("DELIVERY", "Token 与提交"); const tokens = value(bundle.tokens.totals, {}); const git = value(bundle.git.commitStats); root.append(stats([["总 Token", tokens.total, "token"], ["输入", tokens.input, "token"], ["输出", tokens.output, "token"], ["推理", tokens.reasoning, "token"], ["Git 提交", git?.commits ?? "--"]]));
